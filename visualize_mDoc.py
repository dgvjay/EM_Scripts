...
parser=argparse.ArgumentParser(description='''Will parse all the mdoc file present in the input directory (input with -d with \ at end) and generate plots summarizing various attributes of the mdoc files''', epilog="""For questions, contact at digvijay.in1@gmail.com.""")
...
for mdoc_file in glob.glob(mdoc_directory+"*.mdoc"):
 ...
 with open(mdoc_file, "r") as f:
  for line in f:
   ...
   if match3:
    CountsPerElectronList.append(float(match3.group(1)))
   if match4:
    MeanIntensityList.append(float(match4.group(3)))
    MaxIntensityList.append(float(match4.group(2)))
 f.close()
 Increment=abs(TiltAngleList[0]-TiltAngleList[1])
 ...
 MaxbyMeanIntensityList= [aa/bb for aa,bb in zip(MaxIntensityList, MeanIntensityList)]
 fplot = plt.figure(figsize=(6.25,7))
 ax1=fplot.add_subplot(311)
 ax2=fplot.add_subplot(312)
 ax3=fplot.add_subplot(313)
 fig = plt.figure()
 ax1.plot(TiltAngleList, DoseRateOnCameraList, 'o', color='black',label='Through sample(e-/pix/s)',markersize=3)
 ...
 ax2.plot(TiltAngleList, ExposureTimeList, 'o', color='black',label='Exposure Time(s)',markersize=3)
 ...
 ax3.plot(TiltAngleList, MaxbyMeanIntensityList, 'o', color='yellow',label='MaxbyMean Intensity',markersize=3)
